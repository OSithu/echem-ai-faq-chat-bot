...
function App() {
  ...
  const renderTextWithLinks = (text) => {
    ...
    return parts.map((part, idx) => {
      if (urlRegex.test(part)) {
        ...
        return (
          <a key={idx} href={url} target="_blank" rel="noopener noreferrer" style={{ color: '#0066cc', textDecoration: 'underline', cursor: 'pointer' }}>
            {part}
          </a>
        );
      }
        ...
        <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
          {items.map((item, idx) => (
            <li key={idx} style={{ marginBottom: '6px' }}>
              {renderTextWithLinks(item.replace(/^[-•*]\s*/, '').replace(/^\d+\.\s*/, ''))}
            </li>
          ))}
        </ul>
      );
  ...
  const sendMessage = async (messageText = null) => {
    ...
    try {
      ...
      const apiUrl = process.env.REACT_APP_API_URL || `${window.location.origin.replace(':3000', ':8000')}/api/chat`;
      const response = await axios.post(apiUrl, { message: textToSend });
      const botMsg = { role: 'bot', text: response.data.reply };
      setMessages((prev) => [...prev, botMsg]);
    } catch (error) {
      console.error('Chat error:', error);
      const errorMessage = error.response?.data?.reply || 'I apologize, I\'m unable to connect to the server. Please check your internet connection and try again.';
